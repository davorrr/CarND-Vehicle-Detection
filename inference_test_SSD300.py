...
from keras_layers.keras_layer_AnchorBoxes import AnchorBoxes # custom layer that generates Anchor Boxes
from keras_layers.keras_layer_L2Normalization import L2Normalization # Custom layer for L2-Normalization
from keras_layers.keras_layer_DecodeDetections import DecodeDetections  # A custom Keras layer to decode the SSD prediction output. Corresponds to the 'DetectionOutput' layer type in the original Caffe implementation 
from keras_layers.keras_layer_DecodeDetections2 import DecodeDetections2 # More efficient version of 'DetectionOutput' layer type in original Caffe implementation.
from ssd_box_utils.ssd_box_encode_decode_utils import decode_y, decode_y2 # Fuctions for decoding
# raw model output to a format that contains only the positive box predictions. decode_y performs non-maximum
# suppresion per class while decode_y2 does this globally.
#from ssd_batch_generator import BatchGenerator # Batch generator that can perfrom online data augmentation
...
# Now options are to:
# 1. Build the model and then load the trained weights in it, or
...
### If needed implement the loading of trained model also
...
## 1. Load the video
## 2. Make predictions per image
## 3. Construct the new video
...
#### TODO - after inference
## 1. Read Weight Sampling Tutorial
## 2. Read SSD7 training procedure and eventually SSD300 training procedure

## 3. Do the fine tuning (or whole training) of the SSD300 for the Udacity dataset 
##    check if Udacity dataset is already used by Pierlugi Ferarri
